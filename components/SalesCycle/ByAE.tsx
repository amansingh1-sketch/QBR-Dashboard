...
import type { SalesCycleAERow } from "@/lib/types";
...
export default function ByAE({ data, overallAvg }: { data: SalesCycleAERow[]; overallAvg: number }) {
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-2.5 text-right font-medium text-gray-500">Avg Days</th>
              <th className="px-4 py-2.5 text-right font-medium text-gray-500">Median</th>
              <th className="px-4 py-2.5 text-right font-medium text-gray-500">Range</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((row, i) => {
              ...
              return (
                <tr key={row.ownerId} className={i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}>
                  ...
                  <td className="px-4 py-2.5 text-right text-gray-600">{row.medianDays}d</td>
                  <td className="px-4 py-2.5 text-right text-gray-500 text-xs">{row.minDays}–{row.maxDays}d</td>
                </tr>
              );
            })}
          </tbody>
