...
export default function ByAE({ data, selectedTypes }: { data: S1S2Row[]; selectedTypes: Set<DealTypeKey> }) {
  // Apply deal-type filter to each row
  ...
  // Only show AEs with meaningful S1 deal count (filters out SDRs appearing as deal owners)
  const filtered = filteredRows.filter((r) => r.s1Deals >= 10);
