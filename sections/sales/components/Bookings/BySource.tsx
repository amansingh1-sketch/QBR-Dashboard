...
import type { BookingsSourceRow } from "@/sections/sales/types";

const COLORS = ["#6B8CAE", "#8B7FA8", "#06b6d4", "#5B9B8E", "#f59e0b", "#ef4444", "#ec4899", "#84cc16"];
...
export default function BySource({ data, selectedTypes }: { data: BookingsSourceRow[]; selectedTypes: Set<DealTypeKey> }) {
    ...
    <>
      <div className="flex justify-end gap-5 pb-2 text-xs text-gray-600">
        ...
        <div className="flex items-center gap-1.5">
          <span className="inline-block h-0.5 w-4 rounded bg-gray-400" style={{ verticalAlign: "middle" }} />
          <span className="inline-block h-2 w-2 -ml-1.5 rounded-full bg-gray-400" style={{ verticalAlign: "middle" }} />
          {" "}Deals
        </div>
      </div>
      <ResponsiveContainer width="100%" height={260}>
        <ComposedChart data={filtered} margin={{ top: 4, right: 40, left: 8, bottom: 4 }}>
          ...
          <YAxis yAxisId="mrr" orientation="left" tickFormatter={fmtMRR} tick={{ fontSize: 11, fill: "#64748b" }} width={64} />
          <YAxis yAxisId="deals" orientation="right" tick={{ fontSize: 11, fill: "#64748b" }} width={36} />
          <Tooltip content={<CustomTooltip />} />
          <Bar yAxisId="mrr" dataKey="mrr" name="MRR" radius={[4, 4, 0, 0]}>
            {filtered.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
          </Bar>
          <Line yAxisId="deals" type="monotone" dataKey="deals" stroke="#94a3b8" strokeWidth={2}
            dot={{ r: 4, fill: "#94a3b8", strokeWidth: 0 }} activeDot={{ r: 5 }} />
        </ComposedChart>
      </ResponsiveContainer>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-2 text-right font-medium text-gray-500">Avg Deal</th>
