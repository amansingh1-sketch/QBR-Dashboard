  ...
  return (
    <section className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">
            ...
            <code className="rounded bg-gray-100 px-1 text-xs">secondary_win_reasons</code>
            {" "}(multi-select, semicolon-split)
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard
          label={aeRow ? `${aeRow.name} · Won Deals` : "Closed Won Deals"}
          value={fmtNumber(totalDeals)}
          sub={aeRow ? "AE-filtered view" : "With win reason data"}
          accent="emerald"
        />
        <StatCard
          label="Won MRR"
